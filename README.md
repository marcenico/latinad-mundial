...
Dentro de assets se encontrarán con un archivo de nombre **config.json** en el mismo tendrán **acceso a editar** el **token** que se utiliza para el consumo de API.

Además, podrán cambiar el dato de la liga, con el cual se estará obteniendo información. Tambien, existe un dato de configuración, **useMundialMocks**, para utilizar info guardada en los recursos del sitio (esto sirve para hacer pruebas de visualizacion, etc).

Por último existe la **configuración de tiempo y velocidad** en el que se produce el swipe de las diferentes pantallas.
...
**league** : liga de la que se obtendran datos. (int)

**useMundialMocks** : flag para utilizar o no mocks. (boolean)
...
Se recomienda visualizar el sitio en **fullScreen**.
